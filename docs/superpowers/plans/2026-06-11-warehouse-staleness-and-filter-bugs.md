...
- Find where cluster labels should be derived (likely from
  ...
  CONTEXT.md ML decisions) and populate human-readable names instead of
  "undefined".
...
- d1 stat r=+0.141 (imd_score vs trips_per_capita) vs CONTEXT.md ground truth
  ...
  different pairing is intentional (and update CONTEXT.md/figures-registry if a
...
Per `.agent/rules/python-pipeline.md` and `.agent/rules/figures.md`:

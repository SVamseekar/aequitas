...
TITLES: dict[str, str] = {
    ...
    "j2_bcr": "People-gap (no published MKBA €)",
    "j3_carbon": "No published PBL/CBS carbon factor",
    "j4_investment_priority": "Provincie × SES coverage gap",
    "bsa1_franchising_readiness": "OV-wet and provincie concessies",
...
_CARBON_NOTE = "No published PBL/CBS carbon table returned 200. Factor omitted."
...
def _section_bundle(areas, all_areas, region, urban_rural, extras) -> list[dict[str, Any]]:
    ...
    stats_map: dict[str, dict[str, Any]] = {
        ...
        "j2_bcr": {"bcr": None, "omit_euro": True, "reason": "No published MKBA €. People-gap only.", "people_gap": pop_zero, "insufficient_data": False},
        ...
        "bsa1_franchising_readiness": {"national_avg": pct_covered, "unit": "% people within 400 m", "programme": "OV-wet and provincie concessies", "insufficient_data": empty},
        ...
        narratives.update(
            {
                ...
                "c1_route_length": _brief(
                    LENGTH_OMIT if not extras.get("route_length_km") else f"Route-length histogram for this OVapi mode in {place}.",
                    "Kilometres from GTFS shapes. Missing shapes stay a sentence.",
                    caveat_base,
                ),
                "c2_stops_per_route": _brief(
                    ...
                    "Histogram of stops per OVapi route for this mode.",
                    caveat_base,
                ),
                ...
                "j2_bcr": _brief(f"No published MKBA € for {place}. People-gap is {int(pop_zero):,}.", "People-gap only.", caveat_base),
                ...
                "bsa1_franchising_readiness": _brief(f"OV-wet and provincie concessies in {place}: {pct_covered:.1f}% of people within 400 m.", "EU TEN-T / CEF is funding context, not this gap.", caveat_base),
    ...
    if empty:
        ...
        charts = {
            ...
            "c1_route_length": network_charts(
                ...
                stops_title="Stops per OVapi route — {place}",
            )[0],
            ...
            "j2_bcr": {"type": "horizontal_bar", "title": f"People-gap (no published MKBA €) — {place}", "data": [{"label": "People beyond 400 m", "value": pop_zero}]},
            "j3_carbon": {},
            ...
            "bsa1_franchising_readiness": {"type": "choropleth", "geography": geo, "title": f"OV-wet and provincie concessies — {place}", "data": [{"area_code": r["code"], "area_name": r["name"], "value": r["pct_covered"]} for r in province_rows]},
